...
ReturnCode NextHopManager::processAddRequest(
  ...
  P4NextHopEntry next_hop_entry(app_db_entry.next_hop_id,
...
ReturnCode NextHopManager::createNextHop(P4NextHopEntry& next_hop_entry) {
  ...
  CHECK_ERROR_AND_LOG_AND_RETURN(
      sai_next_hop_api->create_next_hop(&next_hop_entry.next_hop_oid, gSwitchId,
                                        ...
                                        next_hop_attrs.data()),
      "Failed to create next hop "
          << QuotedVar(next_hop_entry.next_hop_key) << " on rif "
          << QuotedVar(next_hop_entry.router_interface_id));
...
ReturnCode NextHopManager::removeNextHop(const std::string& next_hop_key) {
  ...
  m_p4OidMapper->decreaseRefCount(
      ...
      KeyGenerator::generateNeighborKey(next_hop_entry->router_interface_id,
