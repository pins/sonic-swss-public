...
AclTableManager::AclTableManager(P4OidMapper* p4oidMapper,
  ...
  if (!status.ok()) {
    SWSS_LOG_ERROR("Failed to create ACL UDF default match : %s",
...
ReturnCode AclTableManager::processAddTableRequest(
  ...
  auto group_it = gSwitchOrch->getAclGroupOidsBindingToSwitch().find(
...
ReturnCode AclTableManager::createUdf(const P4UdfField& udf_field) {
  ...
  if (!m_p4OidMapper->getOID(SAI_OBJECT_TYPE_UDF_MATCH, P4_UDF_MATCH_DEFAULT,
                             &udf_match_oid)) {
    return ReturnCode(StatusCode::SWSS_RC_NOT_FOUND)
           << "UDF default match " << QuotedVar(P4_UDF_MATCH_DEFAULT)
           << " does not exist";
  }
  ...
  CHECK_ERROR_AND_LOG_AND_RETURN(
      sai_udf_api->create_udf(&udf_oid, gSwitchId, (uint32_t)udf_attrs.size(),
                              udf_attrs.data()),
...
ReturnCode AclTableManager::createAclGroupMember(
  ...
  CHECK_ERROR_AND_LOG_AND_RETURN(
      sai_acl_api->create_acl_table_group_member(acl_grp_mem_oid, gSwitchId,
                                                 (uint32_t)acl_mem_attrs.size(),
                                                 acl_mem_attrs.data()),
  ...
  m_p4OidMapper->increaseRefCount(SAI_OBJECT_TYPE_ACL_TABLE_GROUP,
                                  std::to_string(acl_table.stage));
