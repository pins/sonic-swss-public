...
// critical state.
ReturnCode UpdateRouteAttrs(sai_packet_action_t old_action,
                            sai_packet_action_t new_action,
                            sai_object_id_t old_nexthop,
                            sai_object_id_t new_nexthop,
                            const std::string& route_entry_key,
  ...
  route_attr.id = (action_first) ? SAI_ROUTE_ENTRY_ATTR_PACKET_ACTION
  ...
  CHECK_ERROR_AND_LOG_AND_RETURN(
      sai_route_api->set_route_entry_attribute(rotue_entry, &route_attr),
  ...
  // Second attribute
  route_attr.id = (action_first) ? SAI_ROUTE_ENTRY_ATTR_NEXT_HOP_ID
...
ReturnCode RouteManager::validateSetRouteEntry(
  ...
  if (action == p4orch::kSetNexthopId) {
    if (route_entry.nexthop_id.empty()) {
      return ReturnCode(StatusCode::SWSS_RC_INVALID_PARAM)
             << "Empty nexthop_id for route with nexthop_id action";
    }
    if (!route_entry.wcmp_group.empty()) {
      return ReturnCode(StatusCode::SWSS_RC_INVALID_PARAM)
    ...
    if (!route_entry.nexthop_id.empty()) {
      return ReturnCode(StatusCode::SWSS_RC_INVALID_PARAM)
             << "Non-empty nexthop_id for route with drop action";
    }
    if (!route_entry.wcmp_group.empty()) {
      return ReturnCode(StatusCode::SWSS_RC_INVALID_PARAM)
...
ReturnCode RouteManager::createRouteEntry(const P4RouteEntry& route_entry) {
  ...
  if (route_entry.action == p4orch::kSetNexthopId) {
    ...
    CHECK_ERROR_AND_LOG_AND_RETURN(
        sai_route_api->create_route_entry(&sai_route_entry, /*size=*/1,
                                          &route_attr),
    ...
    m_p4OidMapper->getOID(SAI_OBJECT_TYPE_NEXT_HOP_GROUP, wcmp_group_key,
                          &wcmp_group_oid);
    sai_attribute_t route_attr;
    route_attr.id = SAI_ROUTE_ENTRY_ATTR_NEXT_HOP_ID;
...
ReturnCodeOr<sai_object_id_t> RouteManager::getNexthopOid(
    const P4RouteEntry& route_entry) {
  ...
  if (route_entry.action == p4orch::kSetNexthopId) {
    ...
    if (!m_p4OidMapper->getOID(SAI_OBJECT_TYPE_NEXT_HOP, nexthop_key, &oid)) {
      RETURN_INTERNAL_ERROR_AND_RAISE_CRITICAL(
          "Nexthop " << QuotedVar(route_entry.nexthop_id) << " does not exist");
    }
  } else if (route_entry.action == p4orch::kSetWcmpGroupId) {
    auto wcmp_group_key =
        KeyGenerator::generateWcmpGroupKey(route_entry.wcmp_group);
    if (!m_p4OidMapper->getOID(SAI_OBJECT_TYPE_NEXT_HOP_GROUP, wcmp_group_key,
                               &oid)) {
      RETURN_INTERNAL_ERROR_AND_RAISE_CRITICAL(
          "WCMP group " << QuotedVar(route_entry.wcmp_group)
                        << " does not exist");
...
ReturnCode RouteManager::updateRouteEntry(const P4RouteEntry& route_entry) {
  ...
  ASSIGN_OR_RETURN(sai_object_id_t old_nexthop,
                   getNexthopOid(*route_entry_ptr));
  ...
  RETURN_IF_ERROR(UpdateRouteAttrs(
      ...
      (new_route_entry.action == p4orch::kDrop) ? SAI_PACKET_ACTION_DROP
                                                : SAI_PACKET_ACTION_FORWARD,
      old_nexthop, new_nexthop, new_route_entry.route_entry_key,
      &new_route_entry.sai_route_entry));
...
ReturnCode RouteManager::deleteRouteEntry(const P4RouteEntry& route_entry) {
  ...
  auto* route_entry_ptr = getRouteEntry(route_entry.route_entry_key);
  CHECK_ERROR_AND_LOG_AND_RETURN(
      sai_route_api->remove_route_entry(&route_entry_ptr->sai_route_entry),
...
void RouteManager::drain() {
  ...
  for (const auto& key_op_fvs_tuple : m_entries) {
    ...
    if (operation == SET_COMMAND) {
      ...
      if (!status.ok()) {
        SWSS_LOG_ERROR(
            "Validation failed for Set Route APP DB entry with key %s: %s",
            QuotedVar(table_name + ":" + key).c_str(),
            status.message().c_str());
      ...
      if (!status.ok()) {
        SWSS_LOG_ERROR(
            "Validation failed for Del Route APP DB entry with key %s: %s",
            QuotedVar(table_name + ":" + key).c_str(),
            status.message().c_str());
      } else {
