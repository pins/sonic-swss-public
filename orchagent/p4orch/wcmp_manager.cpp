...
ReturnCode WcmpManager::createWcmpGroupMember(
  ...
  CHECK_ERROR_AND_LOG_AND_RETURN(
      sai_next_hop_group_api->create_next_hop_group_member(
          &wcmp_group_member->member_oid, gSwitchId,
...
ReturnCode WcmpManager::createWcmpGroup(P4WcmpGroupEntry* wcmp_group) {
  ...
  // TODO(b/153913503): Update type to WCMP when SAI supports it.
  ...
  CHECK_ERROR_AND_LOG_AND_RETURN(
      sai_next_hop_group_api->create_next_hop_group(
          &wcmp_group->wcmp_group_oid, gSwitchId, (uint32_t)nhg_attrs.size(),
          nhg_attrs.data()),
  ...
  if (port_name_to_wcmp_group_member_map.find(port) !=
    ...
    for (const auto& member : port_name_to_wcmp_group_member_map[port]) {
      ...
      if (it == pruned_wcmp_members_set.end()) {
        ...
        auto status = removeWcmpGroupMember(member, wcmp_group_key);
        if (!status.ok()) {
          SWSS_LOG_NOTICE("Failed to remove member %s from group %s, rv: %s",
                          member->next_hop_id.c_str(),
