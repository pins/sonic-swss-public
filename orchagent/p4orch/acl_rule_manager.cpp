...
ReturnCode AclRuleManager::createAclCounter(const std::string& acl_table_name,
  ...
  if (!m_p4OidMapper->getOID(SAI_OBJECT_TYPE_ACL_TABLE, acl_table_name,
                             &acl_table_oid)) {
    LOG_ERROR_AND_RETURN(ReturnCode(StatusCode::SWSS_RC_NOT_FOUND)
                         << "Invalid ACL counter to create: ACL table key "
                         << QuotedVar(acl_table_name) << " not found.");
  }
  ...
  CHECK_ERROR_AND_LOG_AND_RETURN(
      sai_acl_api->create_acl_counter(counter_oid, gSwitchId,
                                      (uint32_t)counter_attrs.size(),
                                      counter_attrs.data()),
      "Faied to create counter for the rule in table "
          << sai_serialize_object_id(acl_table_oid));
...
ReturnCode AclRuleManager::createAclRule(P4AclRule& acl_rule) {
    ...
    if (acl_rule.counter.counter_oid == SAI_NULL_OBJECT_ID) {
      auto status =
          createAclCounter(acl_rule.acl_table_name, table_name_and_rule_key,
                           acl_rule.counter, &acl_rule.counter.counter_oid);
      if (!status.ok()) {
  ...
  auto sai_status = sai_acl_api->create_acl_entry(
      &acl_rule.acl_entry_oid, gSwitchId, (uint32_t)acl_entry_attrs.size(),
