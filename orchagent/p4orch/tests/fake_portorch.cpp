...
bool PortsOrch::addSubPort(Port& port, const string& alias, const bool& adminUp,
                           const uint32_t& mtu) {
  return true;
}
...
bool PortsOrch::removeVlanMember(Port& vlan, Port& port, string end_point_ip) { return true; }

bool PortsOrch::isVlanMember(Port& vlan, Port& port, string end_point_ip) { return true; }

bool PortsOrch::addVlanFloodGroups(Port &vlan, Port &port, string end_point_ip) { return true; }

bool PortsOrch::removeVlanEndPointIp(Port &vlan, Port &port, string end_point_ip) { return true; }
...
bool PortsOrch::getRecircPort(Port& p, string role) { return true; }

const gearbox_phy_t* PortsOrch::getGearboxPhy(const Port &port) { return nullptr; }

bool PortsOrch::getPortIPG(sai_object_id_t port_id, uint32_t &ipg) { return true; }

bool PortsOrch::setPortIPG(sai_object_id_t port_id, uint32_t ipg) { return true; }
...
bool PortsOrch::addPort(const set<int>& lane_set, uint32_t speed, int an,
                        string fec) {
  return true;
}
...
bool PortsOrch::initPort(const string& alias, const string& role,
                         const int index, const set<int>& lane_set) {
  return true;
}
...
bool PortsOrch::setPortPfcAsym(Port& port, string pfc_asym) { return true; }
...
task_process_status PortsOrch::setPortSpeed(Port& port, sai_uint32_t speed) { return task_success; }

bool PortsOrch::getPortSpeed(sai_object_id_t port_id, sai_uint32_t& speed) {
  return true;
}

bool PortsOrch::setGearboxPortsAttr(Port& port, sai_port_attr_t id,
                                    void* value) {
  return true;
}

bool PortsOrch::setGearboxPortAttr(Port& port, dest_port_type_t port_type,
                                   sai_port_attr_t id, void* value) {
  return true;
}

task_process_status PortsOrch::setPortAdvSpeeds(sai_object_id_t port_id,
...
task_process_status PortsOrch::setPortAutoNeg(sai_object_id_t id, int an) { return task_success; }

bool PortsOrch::setPortFecMode(sai_object_id_t id, int fec) { return true; }

task_process_status PortsOrch::setPortInterfaceType(sai_object_id_t id,
                                     sai_port_interface_type_t interface_type) {
  return task_success;
}

task_process_status PortsOrch::setPortAdvInterfaceTypes(
    sai_object_id_t id, std::vector<uint32_t>& interface_types) {
  return task_success;
}
...
bool PortsOrch::getPortAdvSpeedsVal(const std::string& s,
                                    std::vector<uint32_t>& speed_values) {
  return true;
}

bool PortsOrch::getPortInterfaceTypeVal(
    const std::string& s, sai_port_interface_type_t& interface_type) {
  return true;
}

bool PortsOrch::getPortAdvInterfaceTypesVal(
    const std::string& s, std::vector<uint32_t>& type_values) {
  return true;
}
