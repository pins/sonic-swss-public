...
TEST_F(WcmpManagerTest, ValidateWcmpGroupEntryFailsWhenNextHopDoesNotExist) {
  P4WcmpGroupEntry app_db_entry = {.wcmp_group_id = kWcmpGroupId1,
                                   .wcmp_group_members = {}};
  std::shared_ptr<P4WcmpGroupMemberEntry> gm =
      createWcmpGroupMemberEntry("Unregistered-Nexthop", 1);
...
TEST_F(WcmpManagerTest, ValidateWcmpGroupEntryFailsWhenWeightLessThanOne) {
  P4WcmpGroupEntry app_db_entry = {.wcmp_group_id = kWcmpGroupId1,
                                   .wcmp_group_members = {}};
  std::shared_ptr<P4WcmpGroupMemberEntry> gm =
      createWcmpGroupMemberEntry(kNexthopId1, 0);
  app_db_entry.wcmp_group_members.push_back(gm);
  p4_oid_mapper_->setOID(SAI_OBJECT_TYPE_NEXT_HOP, kNexthopKey1, kNexthopOid1);
...
TEST_F(WcmpManagerTest,
  ...
  P4WcmpGroupEntry app_db_entry = {.wcmp_group_id = kWcmpGroupId1,
                                   .wcmp_group_members = {}};
  std::shared_ptr<P4WcmpGroupMemberEntry> gm =
      createWcmpGroupMemberEntryWithWatchport(kNexthopId1, 1, "EthernetXX",
...
TEST_F(WcmpManagerTest, PruneNextHopSucceeds) {
  ...
  EXPECT_TRUE(VerifyWcmpGroupMemberInPortMap(app_db_entry.wcmp_group_members[0],
                                             true, 1));
  EXPECT_TRUE(VerifyWcmpGroupMemberInPrunedSet(
      app_db_entry.wcmp_group_members[0], false, 0));
  EXPECT_CALL(mock_sai_next_hop_group_,
              remove_next_hop_group_member(Eq(kWcmpGroupMemberOid1)))
      .WillOnce(Return(SAI_STATUS_SUCCESS));
...
TEST_F(WcmpManagerTest,
  ...
  EXPECT_TRUE(VerifyWcmpGroupMemberInPortMap(updated_gm, true, 1));
  EXPECT_TRUE(VerifyWcmpGroupMemberInPrunedSet(
      app_db_entry.wcmp_group_members[0], false, 1));
