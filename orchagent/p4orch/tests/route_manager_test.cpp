...
bool MatchSaiRouteEntry(const sai_ip_prefix_t& expected_prefix,
...
bool MatchSaiAttributeAction(sai_packet_action_t expected_action,
...
bool MatchSaiAttributeNexthopId(sai_object_id_t expected_oid,
                                const sai_attribute_t* attr) {
  if (attr == nullptr) {
    return false;
  }
...
class RouteManagerTest : public ::testing::Test {
 protected:
  ...
  void SetupNexthopIdRouteEntry(const std::string& vrf_id,
    ...
    EXPECT_CALL(mock_sai_route_, create_route_entry(_, _, _))
...
TEST_F(RouteManagerTest, MergeRouteEntryWithNexthopIdActionDestTest) {
  ...
  dest.route_entry_key =
      KeyGenerator::generateRouteKey(dest.vrf_id, dest.route_prefix);
  ...
  src.route_entry_key =
      KeyGenerator::generateRouteKey(src.vrf_id, src.route_prefix);
  P4RouteEntry ret = {};
...
TEST_F(RouteManagerTest, DeserializeRouteEntryWithNexthopIdActionTest) {
  ...
  expect_entry.action = p4orch::kSetNexthopId;
  expect_entry.nexthop_id = kNexthopId1;
  expect_entry.route_entry_key = KeyGenerator::generateRouteKey(
      expect_entry.vrf_id, expect_entry.route_prefix);
...
TEST_F(RouteManagerTest, ValidateSetRouteEntryDoesNotExistInManagerShouldFail) {
  ...
  EXPECT_EQ(StatusCode::SWSS_RC_INVALID_PARAM,
            ValidateSetRouteEntry(route_entry));
}
...
TEST_F(RouteManagerTest, ValidateDelRouteEntryHasActionShouldFail) {
  ...
  route_entry.route_entry_key = KeyGenerator::generateRouteKey(
      route_entry.vrf_id, route_entry.route_prefix);
  EXPECT_EQ(StatusCode::SWSS_RC_INVALID_PARAM,
            ValidateDelRouteEntry(route_entry));
}
...
TEST_F(RouteManagerTest, CreateNexthopIdIpv4RouteSucceeds) {
  ...
  EXPECT_CALL(mock_sai_route_,
              create_route_entry(
                  Truly(std::bind(MatchSaiRouteEntry, sai_ipv4_route_prefix,
                                  std::placeholders::_1, gVrfOid)),
                  Eq(1),
                  Truly(std::bind(MatchSaiAttributeNexthopId, kNexthopOid1,
                                  std::placeholders::_1))))
      .WillOnce(Return(SAI_STATUS_SUCCESS));
  EXPECT_EQ(StatusCode::SWSS_RC_SUCCESS, CreateRouteEntry(route_entry));
...
TEST_F(RouteManagerTest, CreateNexthopIdIpv6RouteSucceeds) {
  ...
  EXPECT_CALL(mock_sai_route_,
              create_route_entry(
                  Truly(std::bind(MatchSaiRouteEntry, sai_ipv6_route_prefix,
                                  std::placeholders::_1, gVrfOid)),
                  Eq(1),
                  Truly(std::bind(MatchSaiAttributeNexthopId, kNexthopOid1,
                                  std::placeholders::_1))))
  ...
  EXPECT_CALL(
      ...
      create_route_entry(
          Truly(std::bind(MatchSaiRouteEntry, sai_ipv4_route_prefix,
                          std::placeholders::_1, gVrfOid)),
          Eq(1),
          Truly(std::bind(MatchSaiAttributeAction, SAI_PACKET_ACTION_DROP,
                          std::placeholders::_1))))
      .WillOnce(Return(SAI_STATUS_SUCCESS));
  EXPECT_EQ(StatusCode::SWSS_RC_SUCCESS, CreateRouteEntry(route_entry));
  VerifyRouteEntry(route_entry, sai_ipv4_route_prefix, gVrfOid);
  ...
  EXPECT_CALL(mock_sai_route_,
              create_route_entry(
                  Truly(std::bind(MatchSaiRouteEntry, sai_ipv4_route_prefix,
                                  std::placeholders::_1, gVrfOid)),
                  Eq(1),
                  Truly(std::bind(MatchSaiAttributeNexthopId, kWcmpGroupOid1,
                                  std::placeholders::_1))))
      .WillOnce(Return(SAI_STATUS_SUCCESS));
  EXPECT_EQ(StatusCode::SWSS_RC_SUCCESS, CreateRouteEntry(route_entry));
  VerifyRouteEntry(route_entry, sai_ipv4_route_prefix, gVrfOid);
...
TEST_F(RouteManagerTest, UpdateRouteFromSetWcmpToSetNextHopSucceeds) {
  ...
  EXPECT_CALL(mock_sai_route_,
              set_route_entry_attribute(
                  Truly(std::bind(MatchSaiRouteEntry, sai_ipv4_route_prefix,
                                  std::placeholders::_1, gVrfOid)),
                  Truly(std::bind(MatchSaiAttributeNexthopId, kNexthopOid2,
                                  std::placeholders::_1))))
      .WillOnce(Return(SAI_STATUS_SUCCESS));
  EXPECT_CALL(
      mock_sai_route_,
      set_route_entry_attribute(
          Truly(std::bind(MatchSaiRouteEntry, sai_ipv4_route_prefix,
                          std::placeholders::_1, gVrfOid)),
          Truly(std::bind(MatchSaiAttributeAction, SAI_PACKET_ACTION_FORWARD,
                          std::placeholders::_1))))
      .WillOnce(Return(SAI_STATUS_SUCCESS));
...
TEST_F(RouteManagerTest, UpdateRouteFromSetNexthopIdToSetWcmpSucceeds) {
  ...
  EXPECT_CALL(mock_sai_route_,
              set_route_entry_attribute(
                  Truly(std::bind(MatchSaiRouteEntry, sai_ipv4_route_prefix,
                                  std::placeholders::_1, gVrfOid)),
                  Truly(std::bind(MatchSaiAttributeNexthopId, kWcmpGroupOid2,
                                  std::placeholders::_1))))
      .WillOnce(Return(SAI_STATUS_SUCCESS));
  EXPECT_CALL(
      mock_sai_route_,
...
TEST_F(RouteManagerTest, UpdateRouteFromNexthopIdToDropSucceeds) {
  ...
  EXPECT_CALL(mock_sai_route_,
              set_route_entry_attribute(
                  Truly(std::bind(MatchSaiRouteEntry, sai_ipv4_route_prefix,
                                  std::placeholders::_1, gVrfOid)),
                  Truly(std::bind(MatchSaiAttributeNexthopId,
                                  SAI_NULL_OBJECT_ID, std::placeholders::_1))))
      .WillOnce(Return(SAI_STATUS_SUCCESS));
  EXPECT_EQ(StatusCode::SWSS_RC_SUCCESS, UpdateRouteEntry(route_entry));
  VerifyRouteEntry(route_entry, sai_ipv4_route_prefix, gVrfOid);
  uint32_t ref_cnt;
...
TEST_F(RouteManagerTest,
       UpdateRouteEntryRecoverFailureShouldRaiseCriticalState) {
  auto swss_ipv4_route_prefix = swss::IpPrefix(kIpv4Prefix);
  SetupNexthopIdRouteEntry(gVrfName, swss_ipv4_route_prefix, kNexthopId1,
                           kNexthopOid1);
...
TEST_F(RouteManagerTest, DeleteRouteEntryWithSaiErrorShouldFail) {
  ...
  EXPECT_CALL(mock_sai_route_, remove_route_entry(_))
      .WillOnce(Return(SAI_STATUS_FAILURE));
  P4RouteEntry route_entry = {};
  route_entry.vrf_id = gVrfName;
  route_entry.route_prefix = swss_ipv4_route_prefix;
...
TEST_F(RouteManagerTest, DeleteIpv4RouteSucceeds) {
  ...
  EXPECT_CALL(mock_sai_route_, remove_route_entry(Truly(std::bind(
                                   MatchSaiRouteEntry, sai_ipv4_route_prefix,
                                   std::placeholders::_1, gVrfOid))))
      .WillOnce(Return(SAI_STATUS_SUCCESS));
  P4RouteEntry route_entry = {};
  route_entry.vrf_id = gVrfName;
...
TEST_F(RouteManagerTest, DeleteIpv6RouteSucceeds) {
  ...
  EXPECT_CALL(mock_sai_route_, remove_route_entry(Truly(std::bind(
                                   MatchSaiRouteEntry, sai_ipv6_route_prefix,
                                   std::placeholders::_1, gVrfOid))))
      .WillOnce(Return(SAI_STATUS_SUCCESS));
  P4RouteEntry route_entry = {};
  route_entry.vrf_id = gVrfName;
...
TEST_F(RouteManagerTest, RouteCreateAndDeleteInDrainSucceeds) {
  ...
  Drain();
  std::string key =
      KeyGenerator::generateRouteKey(gVrfName, swss::IpPrefix(kIpv4Prefix));
...
TEST_F(RouteManagerTest, InvalidCommandInDrainFails) {
  ...
  attributes.push_back(swss::FieldValueTuple{
      prependParamField(p4orch::kNexthopId), kNexthopId1});
  Enqueue(swss::KeyOpFieldsValuesTuple(kKeyPrefix + j.dump(), "INVALID_COMMAND",
                                       attributes));
  Drain();
}
