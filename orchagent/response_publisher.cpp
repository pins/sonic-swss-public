...
void PerformLogRotate() {
  ...
  if (!gResponsePublisherRecordOfs.is_open()) {
    SWSS_LOG_ERROR("Failed to reopen Response Publisher record file %s: %s",
                   gResponsePublisherRecordFile.c_str(), strerror(errno));
  }
}
...
void ResponsePublisher::publish(
  ...
  if (m_notifiers.find(table) == m_notifiers.end()) {
    m_notifiers[table] =
        std::make_unique<swss::NotificationProducer>(&m_db, response_channel);
  }
